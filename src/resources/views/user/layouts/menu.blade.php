@if (Auth::check())
    ...
    <!------------------------------------------------>
    <div id='menu'>
        <img src="{{ asset('storage/images/user/'.Auth::user()->image) }}" alt="{{ Auth::user()->account_id }}">
    </div>
    <ul id='menu-list' class='menu-list hidden'>
        <li>
            <!-- routeの第2引数に連想配列としてパラメータを渡す -->
            <a href="{{ route('profile', ['account_id' => Auth::user()->account_id]) }}">
                <img src="{{ asset('images/close.svg') }}" alt="profile">
                {{ Auth::user()->name }}
            </a>
        </li>
        <li>
            <a href="{{ route('profile.settings', ['account_id' => Auth::user()->account_id]) }}">
